...
export const ClientEditForm = () => {
    ...
    const fetchClientData = async () => {
      try {
        ...
        const formattedClient = {
          ...
          licenseIssueDate: client.licenseIssueDate ? new Date(client.licenseIssueDate).toISOString().split('T')[0] : ''
        };
        
        setFormData(formattedClient);
      } catch (error) {
        console.error('Error fetching client:', error);
        setErrors({ fetch: error.message || 'Failed to load client data' });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate to client details page after successful update
      navigate(`/clients/${id}`);
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="p-4 sm:p-6 max-w-4xl mx-auto flex justify-center items-center h-64">
        <div className="text-center">
          <svg className="animate-spin h-8 w-8 mx-auto text-[#1E265F]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <button
            ...
            aria-label="Go back to client details"
          >
